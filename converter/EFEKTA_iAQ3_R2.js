...
const tzLocal = {
	...
	co2_config: {
        key: ['auto_brightness', 'onoff_brightness', 'forced_recalibration', 'factory_reset_co2', 'long_chart_period', 'long_chart_period2', 'set_altitude', 'manual_forced_recalibration', 'rotate'],
        convertSet: async (entity, key, rawValue, meta) => {
			const endpoint = meta.device.getEndpoint(1);
            const lookup = {'OFF': 0x00, 'ON': 0x01};
            const value = lookup.hasOwnProperty(rawValue) ? lookup[rawValue] : parseInt(rawValue, 10);
...
const definition = {
        ...
        fromZigbee: [fz.temperature, fz.humidity, fz.illuminance, fzLocal.co2, fzLocal.air_quality, fzLocal.co2_config, fzLocal.local_time, fzLocal.co2_gasstat_config],
        toZigbee: [tz.factory_reset, tzLocal.co2_config, tzLocal.co2_gasstat_config],
        ...
        configure: async (device, coordinatorEndpoint, logger) => {
		    ...
		    await reporting.bind(endpoint2, coordinatorEndpoint, ['msIlluminanceMeasurement', 'genAnalogInput']);
        },
