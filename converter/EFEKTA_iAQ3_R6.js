...
const tzLocal = {
	...
	co2_config: {
        key: ['auto_brightness', 'night_onoff_backlight', 'forced_recalibration', 'factory_reset_co2', 'long_chart_period', 'long_chart_period2', 'set_altitude', 'manual_forced_recalibration', 'rotate', 'automatic_scal', 'night_on_backlight', 'night_off_backlight'],
        convertSet: async (entity, key, rawValue, meta) => {
			const endpoint = meta.device.getEndpoint(1);
            const lookup = {'OFF': 0x00, 'ON': 0x01};
            const value = lookup.hasOwnProperty(rawValue) ? lookup[rawValue] : parseInt(rawValue, 10);
	...
	air_quality: {
        ...
        convert: (model, msg, publish, options, meta) => {
            ...
            if (msg.data.hasOwnProperty(0x0065)) {
                result.voc_index = parseFloat(msg.data[0x0065]);
            }
