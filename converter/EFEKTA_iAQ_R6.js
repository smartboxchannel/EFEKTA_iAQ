...
const tzLocal = {
	...
	co2_config: {
        key: ['auto_backlight', 'night_onoff_backlight', 'onoff_brightness', 'forced_recalibration', 'factory_reset_co2', 'long_chart_period', 'set_altitude', 'manual_forced_recalibration', 'rotate', 'automatic_scal', 'night_on_backlight', 'night_off_backlight'],
        convertSet: async (entity, key, rawValue, meta) => {
            const lookup = {'OFF': 0x00, 'ON': 0x01};
			const endpoint = meta.device.getEndpoint(1);
            const value = lookup.hasOwnProperty(rawValue) ? lookup[rawValue] : parseInt(rawValue, 10);
...
const definition = {
        ...
        fromZigbee: [fz.temperature, fzLocal.humidity, fz.illuminance, fzLocal.co2, fzLocal.co2_config,
            fzLocal.temperaturef_config, fzLocal.humidity_config, fzLocal.co2_gasstat_config, fzLocal.node_config],
        toZigbee: [tz.factory_reset, tzLocal.co2_config, tzLocal.temperaturef_config, tzLocal.humidity_config, tzLocal.co2_gasstat_config, tzLocal.node_config],
