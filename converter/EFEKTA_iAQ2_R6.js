...
const tzLocal = {
	...
	co2_config: {
        key: ['auto_brightness', 'onoff_brightness', 'forced_recalibration', 'factory_reset_co2', 'long_chart_period', 'manual_forced_recalibration', 'alarm', 'rotate', 'automatic_scal', 'night_on_backlight', 'night_off_backlight'],
        convertSet: async (entity, key, rawValue, meta) => {
			const endpoint = meta.device.getEndpoint(1);
            const lookup = {'OFF': 0x00, 'ON': 0x01};
            const value = lookup.hasOwnProperty(rawValue) ? lookup[rawValue] : parseInt(rawValue, 10);
...
const definition = {
        ...
        description: '[CO2 Monitor with IPS TFT Display, indoot temperature and humidity, atmosphere pressure, forecast, outdoor temperature, date and time.](http://efektalab.com/iAQ)',
        ...
        exposes: [e.co2(), e.temperature(), e.humidity(),
			...
			exposes.numeric('illuminance_raw', ea.STATE).withUnit('raw').withDescription('Illuminance in raw'),
			e.pressure(),
